...
int R_heatmap(char* directory, std::string filepath, std::string filename, std::string matrix_filename_str){
    ...
    R<<"library(package=grid)\n"
        ...
        "}\n"
        "\n"
        "heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks=c(seq(0,0.75,0.01)), color, legend, annotation, annotation_colors, annotation_legend, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, ...){\n"
        "grid.newpage()\n"
        "\n"
        "# Set layout\n"
        ...
        "dmat<-data.matrix(d)\n"
        "\n"
        "mycol=colorRampPalette(c(\"white\",\"yellow\",\"#FF9B0D\",\"#BD2400\",\"#631300\",\"black\", \"grey60\"))\n"
        "maintitle=tail(strsplit(strsplit(filename, '.fastq')[[1]][1],'/')[[1]],1)\n"
        "maintitle=tail(strsplit(strsplit(maintitle, '.fastq')[[1]][1],'\\\\\\\\')[[1]],1)\n"

        "pheatmap(dmat, cellheight=10, cellwidth=10, cluster_rows=FALSE, filename=paste(filename, '.pdf', sep=''), cluster_cols=FALSE, legend=TRUE, main=maintitle, color=mycol(100), breaks=c(seq(0,1,0.01)))\n"
        "\n";
        ...
        #ifdef _WIN32
            Rcommand = "\"Rscript \""+R_filename_str+"\"\"";
        #else
            Rcommand = "\"\"Rscript \""+R_filename_str+"\"\"\"";
        #endif // OS_WINDOWS
...
int R_histogram(char* directory, std::string filepath, std::string filename, std::string segments_filename_str, double prob_cutoff, std::map <int, int>orig_lengths){
    ...
    for(std::map<int, int >::const_iterator it = orig_lengths.begin();
        it != orig_lengths.end(); ++it)
    {
        lengths<<it->first<< ",";
        counts<<it->second<< ",";
    }
    ...
    R<<"miniplot <- function(fun, x, y=NULL, size=c(1,1), vadj=0.5, hadj=0.5,inset=c(0,0), type=c('plt','fig'), pars=NULL){\n"
        "type <- match.arg(type)\n"
        ...
        "filename <- \""<<segments_filename_str<<"\"\n"
        "cutoff <- "<<prob_cutoff<<"\n"
        "output <- \""<<segments_filename_str<<"_hist\"\n"
        "\n"
        "lengths<-c("<<lengths.str().substr(0, lengths.str().size()-1)<<")\n"
        "counts<-c("<<counts.str().substr(0, counts.str().size()-1)<<")\n"
        "sumcounts=sum(counts)\n\n"

        "d <- read.table(filename, header=T)\n"
        "maxup=max(d[,2])\n"
        "wholecounts=rep(0,max(d[,1]))\n"
        "wholelengths=seq(1,max(d[,1]))\n"
        "for(i in 1:length(lengths)){\n"
        "pos=lengths[i]\n"
        "val=counts[i]\n"
        "wholecounts[pos]=val\n"
        "}\n"
        "bind=cbind(wholelengths,wholecounts)\n"
        "ticks=c(0, rep(NA, max(d$read_length)-2), max(d$read_length))\n"
        ...
        "if (maxup>0.45){\n"
            ...
            "par(op)\n"
            "subplot(barplot(d[,2],col='blue',ylim=lower,xpd=FALSE,las=0, names.arg = d[,1], las=1, axis.lty = 1), x=c(0,1),y=lowspan)\n"
            "myrect=miniplot({rect(par(\"usr\")[1], par(\"usr\")[3], par(\"usr\")[2], par(\"usr\")[4], col = \"white\"); box(lty = 'solid', col = 'black')}, grconvertX(c(.07, .30), \"npc\"), grconvertY(c(.14, .53), \"npc\"))\n"
            "myrect=miniplot({rect(par(\"usr\")[1], par(\"usr\")[3], par(\"usr\")[2], par(\"usr\")[4], ); title(main='Length distribution before cutoff', font.main=1,cex.main=0.6, line=-1)}, grconvertX(c(.07, .30), \"npc\"), grconvertY(c(.14, .53), \"npc\"))\n"

            "miniplot({barplot(bind[,2], xaxt = 'n', mgp = c(3, 1, 0), space = 0, col='grey', las=1, cex.axis = 0.4, cex.names = 0.4); axis(side=1, at=c(0, max(d$read_length)), lwd=0,lwd.ticks = 1, labels = NA, cex.axis=0.5,tcl=-0.1); axis(side=1, at=ticks, lwd = 0, labels = ticks, cex.axis=0.5, line=-1);}, grconvertX(c(.13, .28), \"npc\"), grconvertY(c(.20, .44), \"npc\"))\n"
        ...
        "\n"
        "par(mar=c(8,6,6,3) + 0.1, oma=c(0,0,0,0), mgp = c(3, 1, 0))\n"
        "#lines(bind,type='l', col='grey', lwd=2)\n"
        ...
        "\n"
        "myrect=miniplot({rect(par(\"usr\")[1], par(\"usr\")[3], par(\"usr\")[2], par(\"usr\")[4], col = \"white\" ); box(lty = 'solid', col = 'black')}, grconvertX(c(.04, .27), \"npc\"), grconvertY(c(.22, .63), \"npc\"))\n"
        "myrect=miniplot({rect(par(\"usr\")[1], par(\"usr\")[3], par(\"usr\")[2], par(\"usr\")[4], title(main='Length distribution before cutoff', font.main=1,cex.main=0.6, line=-1), );}, grconvertX(c(.04, .27), \"npc\"), grconvertY(c(.22, .63), \"npc\"))\n"
        "miniplot({barplot(bind[,2], xaxt = 'n', mgp = c(3, 1, 0), space = 0, col='grey', las=1, cex.axis = 0.4, cex.names = 0.4); axis(side=1, at=c(0, max(d$read_length)), lwd=0, lwd.ticks = 1,labels = NA, cex.axis=0.2,tcl=-0.1); axis(side=1, at=ticks, lwd = 0, labels = ticks, cex.axis=0.5, line=-1);}, grconvertX(c(.1, .25), \"npc\"), grconvertY(c(.28, .53), \"npc\"))\n"
        ...
        "mtext(\"Sum of the segments = 1\", 1, line=6)\n"
        "\n"
        "#legend(0.05,0.35,c('Reads', 'Length distribution before cutoff'), cex=0.7, col=c('blue', 'grey'), lty=c(NA, 1), pch=c(15,NA))\n"
        "}\n"
        "garbage<-dev.off()\n";
...
int R_cumulative(char* directory, std::string filepath, std::string filename, std::string segments_filename_str, double prob_cutoff){
    ...
    R<<"filename <- \""<<segments_filename_str<<"\"\n"
        ...
        "d$sumcol[i]=1-sum\n"
		"d$read_length[i]=d$read_length[i]+1}\n"
		"d=rbind(c(0,0,1,1),d)\n"
		"d$ideal[nrow(d)]=0\n"
        "\n"
        "pdf( paste(output,'.pdf', sep = ''), width = 11 )\n"
        "par(mar=c(4.5,6,6,2) + 0.1, oma=c(0,0,0,0), mgp = c(3, 1, 0))\n"
        "plot(x=d$read_length, y=d$sumcol, ylim=c(0,1),panel.first = abline(h=seq(0.0,1,0.2), lty=1, col='grey'),type='l', xlab=paste(\'Length of longest contiguous read segments with quality higher than\',cutoff),col.lab=rgb(0,0.5,0), ylab='Proportion of reads',col.lab=rgb(0,0.5,0), lwd=2, col='purple', las=1)\n"
...
int R_length_summary(char* directory, std::string filepath, std::string filename, std::string second_filepath, std::string second_filename, std::string summary_filename_str, bool paired, int length){
    const char* R_file;
    std::string R_filename_str;
    if(directory){
        R_filename_str=directory+filename+".R";
    ...
    if(paired==true){
        R<<"options(scipen=5)\n"
            ...
            "barX<-barplot(as.integer(dnew[,2]), cex.names=0.8, names.arg=(dnew[,1]), font.lab=2, ylab=\"Reads\", ylim=c(0,uplimit+uplimit/100*20), col=c(\"darkgreen\", \"dodgerblue4\", \"firebrick3\"), beside=TRUE)\n"
            ...
            "Sample1=tail(strsplit(strsplit(filename, '.fastq')[[1]][1],'/')[[1]],1)\n"
            "Sample1=tail(strsplit(strsplit(Sample1, '.fastq')[[1]][1],'\\\\\\\\')[[1]],1)\n"
            "Sample2=tail(strsplit(strsplit(second_filename, '.fastq')[[1]][1],'/')[[1]],1)\n"
            "Sample2=tail(strsplit(strsplit(Sample2, '.fastq')[[1]][1],'\\\\\\\\')[[1]],1)\n"
            "title(paste(\'Sample 1: \',Sample1, \'\\n\', \'Sample 2: \',Sample2,\'\\n\', sep=\'\'), cex.main=1)\n"
            "garbage<-dev.off()\n";
    }else{
        R<<"options(scipen=5)\n"
            "length="<<length<<"\n"
            ...
            "pdf( paste(output, '.pdf', sep = ''))\n"
            "par(mar=c(4.5,6,6,2) + 0.1, oma=c(0,0,0,0), mgp = c(3, 1, 0), cex.lab=1, cex.main=1.3)\n"
            "barX<-barplot(as.integer(dnew[,2]), names.arg=(dnew[,1]), font.lab=2, ylab=\"Reads\", ylim=c(0,uplimit+uplimit/100*20), col=c(\"dodgerblue4\", \"firebrick3\"), beside=TRUE)\n"
            "mtext(paste('sorted length = ',length), 3, line=1)\n"
            ...
            "Title=tail(strsplit(strsplit(filename, '.fastq')[[1]][1],'/')[[1]],1)\n"
            "Title=tail(strsplit(strsplit(Title, '.fastq')[[1]][1],'\\\\\\\\')[[1]],1)\n"
            "title(paste(\'Sample: \',Title), cex.main=1)\n"
            "garbage<-dev.off()\n";
    }
